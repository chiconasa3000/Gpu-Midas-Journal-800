...
{
    //set by default the next parameters
...
::Initialize() throw (ExceptionObject)
{
    ...
    //quantity of fixed images
    const unsigned int FImgTotal = this->GetNumberOfFixedImages();

    //current because is the current images in the level of the pyramid
    ...
    //put only the current fixed image
    this->m_MultiMetric->SetFixedMultiImage( this->m_CurrentFixedMultiImage );
    //the transformation vector but It is null
    ...
    //init the metric
    //initi the multi metric (OJO)
    this->m_MultiMetric->Initialize();

    // Setup the optimizer

    //the function cost is the metric (ojo)
    ...
    //init the output transform with the image.
...
::SetNumberOfLevels( unsigned long numberOfLevels )
{
    ...
    //number of levels specified is only a flag because if is set
    //the planning will be give
    //if is not by default it creates a own planning
...
::PreparePyramids( void )
{
    ...
    //the initial parameters is set in the next level
    ...
    // check number of the fixed images
    const unsigned int FImgTotal = this->GetNumberOfFixedImages();
    if( FImgTotal == 0 )
    {
    ...
    //In this case is a vector where the schedule is set
    if( m_ScheduleSpecified )
    {
        for( unsigned int i=0; i<FImgTotal; i++ )
        {
    ...
    //set the fixed images in the fixed pyramid
    for( unsigned int i=0; i<FImgTotal; i++ )
    {
