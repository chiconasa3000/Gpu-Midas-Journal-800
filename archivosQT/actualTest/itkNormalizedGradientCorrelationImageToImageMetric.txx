...
::Initialize(void) throw ( ExceptionObject )
{
    ...
    for (unsigned int dim=0; dim<m_MaxDimension; dim++)
    {
        ...
        //m_ptrosGpuFixedImages[dim] = GpuFixedImageType::New();

        //PointerFixed miimagenfija = PointerFixed::New();
        //miimagenfija = this->GetFixedImage();

        this->miimage = FixedImageType::New();
        miimage = dynamic_cast<const FixedImageType * >(this->GetFixedImage());
        ...
        m_FixedNeighborFilters[dim] = GpuFixedNeighboorFilterType::New();
        ...
        m_FixedNeighborFilters[dim]->SetInput(dynamic_cast< const GpuFixedImageType * >(this->GetFixedImage()));
        ...
        m_FixedNeighborFilters[dim].Print(std::cout);
    ...
    for (unsigned int dim=0; dim < m_MaxDimension; dim++)
    {
        ...
        m_ptrosGpuMovingImages[dim] = dynamic_cast<const GPUmiMovingImageType *>(m_ResampleImageFilter->GetOutput());
        ...
        m_MovingNeighborFilters[dim]->SetInput(dynamic_cast<const GPUmiMovingImageType *>(m_ResampleImageFilter->GetOutput()));
