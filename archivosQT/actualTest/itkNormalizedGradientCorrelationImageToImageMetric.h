...
//#endif
  ...
  typename FixedImageType::ConstPointer miimage;

  itkStaticConstMacro(FixedImageDimension, unsigned int, TFixedImage::ImageDimension);
  typedef itk::Image<RealType, itkGetStaticConstMacro(FixedImageDimension)> GradientImageType;
	
  //asumimos que la clase GPUTraits ya es declarada y contiene en su interior el tipo imagen GPU
  //pero que como plantilla recibe una imagen normal de ITK

  //Este es el tipo imagen GPU obtenida de la clase GPUTraits
  //la imagen que ingresa es una imagen itk normal la cual es la que se desea convertir
  typedef typename itk::GPUTraits< TFixedImage >::Type GPUmiFixedImageType;
  typedef typename itk::GPUTraits< TMovingImage >::Type GPUmiMovingImageType;

  //declaramos los punteros constantes aunque deseamos que estos sean solo normales
  //probaremos esto despues
  typename GPUmiFixedImageType::ConstPointer miConstPtroGpuFixedImage;
  typename GPUmiMovingImageType::ConstPointer miConstPtroGpuMovingImage;
  ...
  typedef GPUImage< MovingImagePixelType, itkGetStaticConstMacro(MovingImageDimension)> GpuMovingImageType;

  typedef GPUImage< RealType, itkGetStaticConstMacro(MovingImageDimension)> OutputImageType;
  ...
  typename GpuMovingImage::ConstPointer m_ptrosGpuMovingImages[ itkGetStaticConstMacro( MovingImageDimension ) ];
  typename GpuMovingImage::ConstPointer m_ptrosGpuOutputImages[ itkGetStaticConstMacro( MovingImageDimension ) ];
  ...
  //typedef FixedImagePixelType RealFixedOutputPixelType;
  //typedef typename itk::NumericTraits<RealFixedOutputPixelType>::ValueType RealFixedOutputPixelValueType;

  //typedef FixedImagePixelType RealMovingOutputPixelType;
  //typedef typename itk::NumericTraits<RealMovingOutputPixelType>::ValueType RealMovingOutputPixelValueType;
...
protected:
  ...
  //actualmente tiene como tipo de imagen de salida una imagen de itk
  //para una imagen de GPU usar outputimagetype
...
private:
  ...
  typename GpuFixedNeighboorFilterType::Pointer m_FixedNeighborFilters[ itkGetStaticConstMacro( FixedImageDimension ) ];
  typename GpuMovingNeighboorFilterType::Pointer m_MovingNeighborFilters[ itkGetStaticConstMacro( MovingImageDimension ) ];
  ...
  ZeroFluxNeumannBoundaryCondition<GpuMovingImage> m_MovingBoundaryCondition;//change
